...
op.set_defaults(
    ...
    hamlib_interval         = 0.1,      # Wait between hamlib freq. checks (secs)    
    hamlib_rigtype          = 122,      # FT857==122. IC-706==309.
    ...
    pulse                   = 100,       # pulse clip threshold
    rev_iq                  = True,    # Reverse I & Q
    rtl_frequency           = 68330500,   # RTL center freq. Hz  #ft-857 IF Freq
    #rtl_frequency           = 68332196,   # RTL center freq. Hz #ic-706 IF Freq
    rtl_gain                = 45,        # auto
    sample_rate             = DEF_SAMPLE_RATE,    # (stereo) frames/second (Hz)
    si570_frequency         = 7050.0,   # initial freq. for Si570 LO.
    size                    = 512,      # size of FFT --> freq. resolution
    ...
    v_min                   =-80,      # palette starts at this level
    v_max                   =-20,       # palette ends at this level
    waterfall               = True,    # Using waterfall? T/F
    waterfall_accumulation  = 1,        # No. of spectra per waterfall line
...
opt.ident = "FT857D Panadapter de DS3DND"
