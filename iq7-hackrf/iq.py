...
if opt.hamlib:
    ...
    def updatefreq(interval, rig):
        ...
        rigfreq = int(rig.get_freq())      # freq in kHz
        while True:                     # forever!
            ...
            if rigfreq_request:         # ordering of loop speeds up freq change
                if rigfreq_request != rigfreq:
                    ...
                    rigfreq_request = None
            rigfreq = int(rig.get_freq()) / 10     # freq in MHz
            time.sleep(interval)
...
if opt.lcd4:                        # setup for directfb (non-X) graphics
    ...
    SCREEN_SIZE = (1300,700) if opt.waterfall \
                     else (1300,480) # NB: graphics may not scale well
...
# Waterfall geometry
h_wf = SCREEN_SIZE[1] - (y_2d + h_2d) - 10      # Height of waterfall (3d spectrum)
...
lgfont = pg.font.Font("Patopian 1986.ttf", 120) # normal 16
lgfont_ht = lgfont.get_linesize()       # text height
mefont = pg.font.Font("BITSUMIS.TTF", 30)
mefont_ht = mefont.get_linesize()

mzfont = pg.font.Font("BITSUMIS.TTF", 40)
...
if (opt.control == "yaesu") and opt.hamlib:
    print ("Warning: Hamlib requested with si570.  Si570 wins! No Hamlib.")
if opt.hamlib and (opt.control != "yaesu"):
    import Hamlib
...
def button(msg,x,y,w,h,ic,ac,action=None):
    ...
    textSurf, textRect = text_objects(msg, mzfont)
...
while True:
    ...
    if showfreq:
        ...
        ctl_box = pg.Surface((SCREEN_SIZE[0]-10, 210) )
        ctl_box.fill(SCREEN)
        ...
        surf_main.blit(lgfont.render(msg, 1, BLACK, SCREEN), 
                            (w_middle + x_spectra - ww/2, y_2d-160))
        msg = "Mhz"
        surf_main.blit(mzfont.render(msg, 1, BLACK, SCREEN),
                       (w_middle + x_spectra+ww/2 , y_2d-100))
        
        msg = "MODE %s" % (Hamlib.rig_strrmode(mode))
        surf_main.blit(mefont.render(msg, 1, BLACK, SCREEN), (40,20))
    ...
    button("SETUP",1050,160,140,50,SCREEN,BLUE,quit_all)
    button("QUIT",1190,160,100,50,SCREEN,BLUE,quit_all)
